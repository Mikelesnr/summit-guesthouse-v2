...
export default function Search({ initialSearch = null }: SearchProps) {
    ...
    const [search, setSearch] = useState<AvailabilitySearch | null>(
        initialSearch?.check_in && initialSearch?.check_out
            ? {
                  check_in: initialSearch.check_in,
                  check_out: initialSearch.check_out,
                  party_size: initialSearch.party_size ?? 1,
              }
            : null,
    );
    ...
    const cartTotal = useMemo(() => {
        ...
        return cartItems.reduce(
            (sum, item) =>
                sum + Number(item.room.price) * item.quantity * nights,
            0,
        );
    }, [cartItems, search]);
    ...
    async function runSearch(params: AvailabilitySearch) {
        ...
        try {
            ...
            setError(
                "Couldn't check availability — please try again, or message us on WhatsApp.",
            );
        } finally {
            setLoading(false);
        }
        ...
        <SiteLayout>
            ...
            <section className="mx-auto max-w-6xl px-6 py-16">
                ...
                <p className="mt-2 max-w-lg text-sm text-ink/60">
                    Planning for a group or a conference? Enter your total party
                    size — we&apos;ll help you put together enough rooms to fit
                    everyone.
                </p>
                    ...
                    <div className="mt-12 grid grid-cols-1 gap-10 lg:grid-cols-3">
                        <div className="lg:col-span-2">
                            <h2 className="font-display text-xl text-ink">
                                {rooms.length > 0
                                    ? `${rooms.length} room type${rooms.length > 1 ? 's' : ''} to choose from`
                                    : 'Nothing free for those dates'}
                            </h2>
                            <p className="mt-1 text-sm text-ink/50">
                                Each room fits up to 2 guests — for{' '}
                                {search.party_size} guest
                                {search.party_size > 1 ? 's' : ''}, you&apos;ll
                                likely need at least {suggestedRooms} room
                                {suggestedRooms > 1 ? 's' : ''}.
                            </p>

                            {rooms.length === 0 ? (
                                <p className="mt-4 text-sm text-ink/60">
                                    Try different dates, or message us on
                                    WhatsApp — we sometimes have last-minute
                                    changes.
                                </p>
                            ) : (
                        ...
                        <div className="h-fit rounded-2xl border border-line bg-white p-6 shadow-card">
                            ...
                            {cartItems.length === 0 ? (
                                <p className="mt-3 text-sm text-ink/50">
                                    Add rooms from the list to build your
                                    booking.
                                </p>
                            ) : (
                            ...
                            <div className="mt-4 border-t border-line pt-4">
                                <p className="flex justify-between text-sm text-ink/60">
                                    <span>Covers</span>
                                    <span>
                                        {guestsCovered} of {search.party_size}{' '}
                                        guests
                                    </span>
                                </p>
                            ...
                            {guestsCovered < search.party_size &&
                                cartItems.length > 0 && (
                                    <p className="mt-3 text-xs text-gold-dark">
                                        This only covers {guestsCovered} of{' '}
                                        {search.party_size} guests — add another
                                        room, or continue if that&apos;s
                                        intentional.
                                    </p>
                                )}
                ...
                {step === 'details' && search && cartItems.length > 0 && (
                    <div className="mt-12 max-w-xl">
                        <button
                            ...
                            className="mb-4 text-sm text-ink/50 hover:text-ink"
                        >
