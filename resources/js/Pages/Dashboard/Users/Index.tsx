    ...
    return (
        <DashboardLayout>
            ...
            <p className="mt-1 text-sm text-ink/60">
                You can only see and manage roles at or below your own —{' '}
                {ROLE_LABEL[auth.user.role]} sees{' '}
                {assignableRoles
                    .map((r) => ROLE_LABEL[r])
                    .join(', ')
                    .toLowerCase()}
...
function NewUserForm({ assignableRoles }: { assignableRoles: UserRole[] }) {
        ...
        router.post('/dashboard/users', form, {
            ...
            onSuccess: () => {
                setForm({
                    ...
                    phone: '',
                    role:
                        assignableRoles[assignableRoles.length - 1] ?? 'staff',
                    password: '',
                });
                setOpen(false);
            },
    ...
    return (
        <form
            onSubmit={submit}
            className="mt-6 grid grid-cols-1 gap-3 rounded-2xl border border-line bg-white p-6 shadow-card sm:grid-cols-2"
        >
    ...
    return (
        <tr>
            <td className="px-4 py-3 font-medium text-ink">
                {user.name}{' '}
                {isSelf && <span className="text-xs text-ink/40">(you)</span>}
            </td>
